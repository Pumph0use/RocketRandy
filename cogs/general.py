...
class General(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        if channel is not None:
            with open('resources/responses.json', 'r') as in_file:
                ...
                response = random.choice(responses['greetings']).replace('%MEMBER%', member.mention)
    ...
    async def general_on_message(self, message):
        ...
        if message.author == self.bot.user:
            ...
            await message.channel.send(f'No. Fuck you {message.author.mention}, my good sir/madam!')

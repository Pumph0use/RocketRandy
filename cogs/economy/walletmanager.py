...
class WalletManager(commands.Cog):
    ...
    async def wallet(self, ctx, member: discord.Member = None):
        ...
        await ctx.send(f"{member.mention} You have {member_wallet['contents']['basic_currency']} credits")
    ...
    async def test_add(self, ctx, amount: int, member: discord.Member = None):
        ...
        await ctx.send(f"Your wallet has been updated to {member_wallet['contents']['basic_currency']}")
    ...
    def add_wallet(self, member: discord.Member):
        member_wallet = {'member_id': member.id,
                         'contents': {
                             'basic_currency': 1000
                         }}

...
async def write_response_to_file(ctx, response, platform, username, file_name):
    ...
    await ctx.send(f'{ctx.author.mention} here is your requested data!', file=discord.File(out_file))


async def log_request_error(ctx, request):
    logging.info(f'Received {request.status} from Tracker Network!!!')
    await ctx.send(f"I'm so sorry {ctx.author.mention}. "
...
class RocketLeague(commands.Cog):
    ...
    @commands.command()
    @commands.is_owner()
    async def set_role(self, ctx, platform: str, username: str, member: discord.Member = None):
            ...
            if results['data']:
                ...
                if results['data']['segments']:
                    ...
                    if best_rank not in [role.name for role in ctx.guild.roles]:
                        ...
                        new_role = await ctx.guild.create_role(name=best_rank, mentionable=True)
                        await member.add_roles(new_role)
                    else:
                        for role in ctx.guild.roles:
                            if role.name == best_rank:
                                if member not in role.members:
                                    self.logger.info(f'Adding {member.name} to {role.name}...')
                                    await member.add_roles(role)

                    await ctx.send(f'{member.mention} Your best playlist ID is '
                                   f'(ID: {best_playlist_id}) {best_playlist_name}, '
                                   f'and you have been assigned the rank role of {best_rank}')
            else:
                await ctx.send(f"{member}, I'm truly sorry sir/madam, "
    ...
    async def rl(self, ctx):
        await ctx.send(f'{ctx.author.mention} please try !help for information on things I can do.')

    @rl.command()
    async def sessions(self, ctx, platform: str, username: str, member: discord.Member = None):
        member = member or ctx.author
    ...
    async def mmr(self, ctx, platform: str, username: str, member: discord.Member = None):
        member = member or ctx.author
        ...
        if response['data']:
            ...
            if response['data']['segments']:
                ...
                await ctx.send(f'{member.mention} Your best playlist is '
                               f'(ID: {best_playlist_id}) {best_playlist_name}, '
                               f'and your MMR is {max_mmr}')

    @mmr.command()
    async def dl(self, ctx, platform: str, username: str, member: discord.Member = None):
        member = member or ctx.author
    ...
    @mmr.command()
    async def threes(self, ctx, platform: str, username: str, member: discord.Member = None):
        member = member or ctx.author
    ...
    async def get_user_info(self, ctx, platform: str, username: str):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f'{RL_TRACKER_URL}/standard/profile/{platform}/{username}') as request:
    ...
    async def get_user_sessions(self, ctx, platform: str, username: str):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f'{RL_TRACKER_URL}/standard/profile/{platform}/{username}/sessions') as request:
                if request.status == 200:
                    return await request.json()
                else:
                    return await log_request_error(ctx, request)

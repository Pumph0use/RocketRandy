...
async def on_ready():
    ...
    if AVATAR_HASH != disc_bot_client.user.avatar:
        ...
        set_key('.env', 'AVATAR_HASH', await utils.set_avatar(disc_bot_client, avatar_path))
    else:
...
async def reload(ctx, extension_name: str):
    ...
    await ctx.send(f'{ctx.author.mention} I have reloaded the {extension_name} extension.')
